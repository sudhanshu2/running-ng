...
## Usage
```console
runbms [-h|--help] [-i|--invocations INVOCATIONS] [-s|--slice SLICE] [-p|--id-prefix ID_PREFIX] [-m|--minheap-multiplier MINHEAP_MULTIPLIER] [--skip-oom SKIP_OOM] [--skip-timeout SKIP_TIMEOUT] [--resume RESUME] [--workdir WORKDIR] [--skip-log-compression] [--skip-env-dump] [--exit-on-failure CODE] [--randomize-configs] LOG_DIR CONFIG [N] [n ...]
```
...
`--skip-env-dump`: skip dumping all environment variables into the log prologue.
